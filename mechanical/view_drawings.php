...
$sql = "SELECT * FROM project p JOIN mechdrawing mcd ON p.project_id = mcd.project_id WHERE p.project_id = '{$project['project_id']}'";
$drawings = select_rows($sql);

function renderFile($file, $title) {
    $ext = pathinfo($file, PATHINFO_EXTENSION);
    if (in_array($ext, ['jpg', 'jpeg', 'png', 'gif'])) {
        return "<img src='" . file_url . $file . "' style='width:150px; height:auto; border-radius:5px;' title='$title' alt='$title'>";
    } elseif ($ext === 'pdf') {
    ...
    <div class="card">
        <div class="card-datatable table-responsive">
            <table class="datatables-basic table border-top">
                ...
                <tbody>
                    <?php $cnt = 1; foreach ($drawings as $drawing): ?>
                        <tr>
                            <td></td>
                            <td><?= $cnt ?></td>
                            <td><?= renderFile($drawing['structural_pdf'], 'Structural Pdf') ?></td>
                            <td><?= renderFile($drawing['strctural_file'], 'Structural File') ?></td>
                            ...
                            <td>
                                <a href="?id=<?= encrypt($drawing['project_id']) ?>" class="btn btn-primary">
                                    <i class="fas fa-pencil"></i> Edit
                                </a>
                                <a href="<?= base_url ?>model/delete?id=<?= encrypt($drawing['structdrawing_id']) ?>&table=<?= encrypt('structdrawing') ?>&page=<?= encrypt('view_drawings') ?>&method=drawing" class="btn btn-danger mt-1">
